...
def is_valid_url(url):
    if url.find('#') != -1 or url.find('javascript:') != -1:
        ...
        ans = re.match(ur'http://zu.soufun.com/house/i[\d]+/?', url) != None or re.match(ur'http://zu.soufun.com/chuzu/[-_\d\w]+\.htm', url) != None
    #print ans
    return ans
...
def analyse(page):
    ...
    if m == None or len(m.group(2)) > g_max_arch_length:
        print 'No arch'
        return None
    arch = m.group(2)
